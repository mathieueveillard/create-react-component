...
async function createComponentTextFromTemplate(): Promise<string> {
  const template = await promises.readFile(`${__dirname}/templates/component/component.handlebars`, "utf-8");
  const compiledTemplate = Handlebars.compile(template);
  return compiledTemplate({});
}

async function createFolderAndFile(folderName: string, fileNameWithExtension: string, text: string): Promise<void> {
